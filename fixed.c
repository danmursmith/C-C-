...
void ST7735_Line(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color){
	// TODO - validate input
	uint16_t xcurrent = x1;
	uint16_t ycurrent = y1;
	uint16_t snumerator = y2 - ycurrent;
	uint16_t sdenominator = x2 - xcurrent;
	uint16_t divisor;																						
	if (sdenominator < snumerator){
		divisor = sdenominator;
	} else {
		divisor = snumerator;
	}
	while (snumerator%divisor!= 0 && sdenominator%divisor != 0 && divisor > 1){
		divisor--;
	}
	snumerator = snumerator / divisor;
	sdenominator = sdenominator / divisor;
	while(xcurrent != x2 && ycurrent != y2){
		ST7735_DrawPixel(xcurrent, ycurrent, color);
		xcurrent = xcurrent+ sdenominator;
		ycurrent = ycurrent+ snumerator;
	} 
